...
size = 30  # 1セルの幅（描画にしか関係ない）
amo = 20  # セルの縦横の数（初期10）
wait = 0  # 0で毎回ユーザからの操作待ち、1以上でその秒数待つ（描画）
start_x = 1 # スタートの座標x（初期9）※0.2追記始終点入替
start_y = 15  # スタートの座標y（初期5）※0.2追記始終点入替
goal_x = 17  # ゴールの座標x※0.2追記始終点入替
goal_y = 4  # ゴールの座標y※0.2追記始終点入替
sv_count = -1  # 画像保存用：-1で表示のみ※0.2追記
dirName = 'img'
os.makedirs(dirName, exist_ok=True)

next = []  # 次訪問したいリスト[x,y]
dp = [[[0.0, 0.0] for i in range(amo)] for j in range(amo)]  # [cost, visited]
...
def rect(stx, sty, col):
    x = stx * size+(int)(size/2)
    y = sty * size+(int)(size/2)
    #cv2.rectangle(img, (x+4, y+4), (x+size, y+size), color=col, thickness=-1)
    cv2.circle(img, (x+2,y+2), (int)(size/4), col, thickness=-1)

# 文字
def chara(x, y):
    cv2.putText(img, str(dp[x][y][0]), ((int)((x+0.5)*size), (int)((y+1.2)*size)),
...
obstInit(2, 4, 4, 18)  # 障害物
obstInit(8, 0, 12, 17)  # 障害物
sv_count = show(sv_count)

def minDist(stx,sty,x,y,p):
    if dp[x][y][0]==0:
        dp[x][y] = [dp[stx][sty][0]+p, 10]
    else:
        dp[x][y] = [min(dp[stx][sty][0]+p,dp[x][y][0]), 10]
    ...
    for i in range(-1, 2):
        ...
        for j in range(-1, 2):
            ...
            if(y < 0 or y >= amo):  # 範囲外
                ...
                continue
            elif(dp[x][y][1] % 5 == 1):  # スタート(1)または障害物(6)または訪問済みゴール(16)
                continue
            elif(dp[x][y][1] == 2):  # ゴール(2)
                minDist(stx,sty,x,y,1)
                dp[x][y][1] = 16
                chara(x, y)
                continue
            if (i % 2 == 1 and j % 2 == 1):  # 以下2行追記（斜めスキップ）
                minDist(stx,sty,x,y,1.5)#訪問済み(10)
            else:
                minDist(stx,sty,x,y,1)  # 訪問済み
            rect(x, y, (180, 130, 70))
...
route = [[] for i in range((int)(dp[start_x][start_y][0]+1))]
